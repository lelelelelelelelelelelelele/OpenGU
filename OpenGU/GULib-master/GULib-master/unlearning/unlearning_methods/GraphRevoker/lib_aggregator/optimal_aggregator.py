...
class OptimalAggregator:
    ...
    def _generate_train_data(self):
        ...
        if self.args["num_opt_samples"] == 100:
            selected_indices = np.random.choice(np.arange(len(train_indices)), size=100, replace=False)
        elif self.args["num_opt_samples"] == 10000:
            selected_indices = np.random.choice(np.arange(len(train_indices)), size=int(train_indices.shape[0] * 0.1), replace=False)
        elif self.args["num_opt_samples"] == 1:
            selected_indices = np.random.choice(np.arange(len(train_indices)), size=int(train_indices.shape[0]), replace=False)
