...
class d2dgn():
    ...
    def run_exp(self):
        ...
        " - Average Attack AUC Score: {:.4f} ± {:.4f} seconds\n".format(
            BLUE_COLOR,
    ...
    def unlearn(self):
        if self.args["unlearn_task"]=="node":
            # Randomly select nodes to unlearn
            ...
            self.data.keep_mask = torch.from_numpy(np.isin(np.arange(self.data.num_node), self.data.keep_indices))
            self.data.unlearn_mask = torch.from_numpy(np.isin(np.arange(self.data.num_node), self.data.unlearn_indices))
            ...
            self.data.keep_mask = torch.from_numpy(np.isin(np.arange(self.data.num_node), self.data.train_indices))
            self.data.unlearn_mask = torch.from_numpy(np.isin(np.arange(self.data.num_node), self.data.train_indices))
            

            edge_indices = np.arange(self.data.edge_index.size(1))
            unlearn_edge_indices = np.random.choice(edge_indices, self.unlearning_number, replace=False)
            
            # Create masks for the edges to keep and to unlearn
            keep_edge_mask = ~np.isin(edge_indices, unlearn_edge_indices)
            unlearn_edge_mask = np.isin(edge_indices, unlearn_edge_indices)
            
            # Update edge_index to remove unlearned edges
            self.data.keep_edge_index = self.data.edge_index[:, keep_edge_mask]
            self.data.unlearn_edge_index = self.data.edge_index[:, unlearn_edge_mask]
