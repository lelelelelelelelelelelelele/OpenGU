...
class gukd():
    ...
    def unlearn(self):
        ...
        if self.args["downstream_task"]=="node":
            unlearn_nodes = np.random.choice(np.arange((self.data.num_node)), self.unlearning_number, replace=False)
            keep_mask = np.ones(self.data.num_node, dtype=bool)
            ...
            self.retain_data.train_mask = torch.from_numpy(np.isin(np.arange(self.retain_data.num_node), self.retain_data.train_indices))
            ...
            # 获取训练集节点的索引
            train_indices = self.retain_data.train_indices

            # 找到这些节点相关的边
            edge_index = self.retain_data.edge_index.numpy()
            train_edges_mask = np.isin(edge_index[0], train_indices) | np.isin(edge_index[1], train_indices)

            # 获取训练集部分的边的索引
            train_edges_indices = np.where(train_edges_mask)[0]

            # 随机选择要删除的边的索引
            if len(train_edges_indices) > self.unlearning_number:
                remove_edges_indices = np.random.choice(train_edges_indices, self.unlearning_number, replace=False)
            else:
                remove_edges_indices = train_edges_indices
    ...
    def pre_train(self):
        ...
        self.teacher_model = get_trainer(self.args,self.logger,teacher_model,self.retain_data)
