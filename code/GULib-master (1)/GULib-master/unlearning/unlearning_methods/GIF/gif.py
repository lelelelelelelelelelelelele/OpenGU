...
from utils.dataset_utils import save_train_test_split
...
class gif:
    def __init__(self,args,dataset,logger,model_zoo):
        self.args = args
        self.model_zoo = model_zoo
        self.data,self.dataset= self.model_zoo.data,dataset
        self.logger = logger
    ...
    def GraphInfluenceFunction(self):
        ...
        for run in range(self.args['num_runs']):
            ...
            run_training_time, result_tuple = self._train_unlearning_model(run)
    ...
    def gen_train_graph(self):
        self.logger.debug("Before deletion. train data  #.Nodes: %f, #.Edges: %f" % (
            self.data.num_node, self.data.num_edge))
    ...
    def train_test_split(self):
        if self.args['is_split']:
            ...
            if self.data.name in ['ogbn-arxiv', 'ogbn-products']:
                ...
                self.train_indices, self.test_indices = train_test_split(np.arange((self.data.num_node)),
            ...
            self.data.train_mask = torch.from_numpy(np.isin(np.arange(self.data.num_node), self.train_indices))
            self.data.test_mask = torch.from_numpy(np.isin(np.arange(self.data.num_node), self.test_indices))
        else:
    ...
    def _train_unlearning_model(self, run):
        self.logger.info('training target models, run %s' % run)

        start_time = time.time()
        self.target_model.data = self.data
        # res = self.target_model.GIF_train(
    ...
    def unlearning_request(self):
        ...
        if self.args["downstream_task"]=="node":
            ...
            self.find_k_hops_edge(unique_nodes,unique_edges)
    ...
    def find_k_hops_edge(self,unique_nodes,unique_edges):
        ...
        for col in unique_edges.T:
            neighbor_edges = neighbor_edges[:, ~np.all(neighbor_edges == col[:, None], axis=0)]
