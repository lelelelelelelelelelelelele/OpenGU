...
def self_lpa(G, label_number, iter, seed=42):
    '''
    heritaged from label_propagation_communities
    use G, color_number, iter as input
    '''
    random.seed(seed)
    coloring = _color_network(G)
    # Create a unique label for each node in the graph
    labeling = {node: random.randint(0, label_number - 1) for node in G}
    i = 0
    while not _labeling_complete(labeling, G) and i < iter:
