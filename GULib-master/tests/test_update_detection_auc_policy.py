...
def test_primary_yaml_configs_make_dataset_scope_explicit():
    small = yaml.safe_load(
        (REPO_ROOT / "docs/archive/experiment-configs-pre-aagu034/phase_b_cora_gcn.yaml").read_text(
            encoding="utf-8"
        )
    )
    large = yaml.safe_load(
        (REPO_ROOT / "docs/archive/experiment-configs-pre-aagu034/phase_b_arxiv_T1_seed42.yaml").read_text(
            encoding="utf-8"
        )
    )
...
def test_every_arxiv_phase_config_disables_optional_auc():
    for path in (REPO_ROOT / "docs/archive/experiment-configs-pre-aagu034").glob("phase_b_arxiv*.yaml"):
