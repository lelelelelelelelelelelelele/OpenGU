...
def render_status_markdown(
    events: Sequence[Mapping[str, Any]], warnings: Sequence[str], max_cells: int = 200
) -> str:
    ...
    lines.extend(
        [
            "",
            "The historical `auto_report.md` remains append-only v1/v2 evidence and is not copied into this view.",
            "",
        ]
    )
    return "\n".join(lines)
...
<title>AutoReport current status</title>
<style>
...
<body><main>
<h1>AutoReport current status</h1>
...
__WARNING_HTML__
<footer>The historical <code>auto_report.md</code> remains append-only v1/v2 evidence and is not copied into this view.</footer>
</main></body></html>
"""
    replacements = {
        "__CARDS__": cards,
...
def write_status_views(
    *,
    ...
    markdown = render_status_markdown(events, warnings, max_cells=max_cells)
    browser = render_status_html(events, warnings, max_cells=max_cells)
