...
SMALL_SELECTION_GU_V4_RECIPE_INTRODUCED_SHA = "544eab7ece867a406a4bdc703f7e4c10bb9a313c"
SMALL_SELECTION_GU_V5_RECIPE_INTRODUCED_SHA = "324d3a0434614ec0d206e18f784560ae90f5f945"
...
SMALL_SELECTION_GU_V2_OUTPUT_ROOT = "results/runs/__syncmate_small_selection_gu_v2__"
...
SMALL_SELECTION_GU_V4_OUTPUT_ROOT = "results/runs/__syncmate_small_selection_gu_v4__"
...
SMALL_SELECTION_GU_V5_OUTPUT_ROOT = "results/runs/__syncmate_small_selection_gu_v5__"
...
SMALL_SELECTION_GU_V4_FULL_OUTPUT_ROOT = "results/runs/__syncmate_small_selection_gu_full_v4__"
SMALL_SELECTION_GU_V4_FULL_CONFIG = "experiments/configs/syncmate_small_selection_gu_full_v4.yaml"
SMALL_SELECTION_GU_V4_FULL_CONFIG_SHA256 = "551c5360ec424b1eb35d6c6fce06f251bfe01c3380a0fa731cb136c8e3022f21"
SMALL_SELECTION_GU_V5_FULL_OUTPUT_ROOT = "results/runs/__syncmate_small_selection_gu_full_v5__"
SMALL_SELECTION_GU_V5_FULL_CONFIG = "experiments/configs/syncmate_small_selection_gu_full_v5.yaml"
SMALL_SELECTION_GU_V5_FULL_CONFIG_SHA256 = "bdabc12b1a1cb83938c21eeb3b0e899d80855af38f036e38d08186a1ae4451dd"
...
TARGET_DIRECT_CONFIG_SHA256 = "7aba7def2f5cd4a4690a0419c0a87741ffa94786ac7a32229efd68db01ba781d"
...
def _target_direct_selection_recipe(stage: str) -> Dict[str, Any]:
    ...
    return {
        ...
        "argv": (
            ...
            "--action", "selection", "--stage", stage, "--json",
        ),
        ...
        "expected_artifact_paths": _target_direct_selection_artifacts(stage),
        "success_predicate": (
        ...
        "selection_matrix": {
            "stage": stage,
            "datasets": (TARGET_DIRECT_DATASET_DISPLAY[dataset],),
            "seeds": (seed,),
            "score_count": 17,
...
def _target_direct_gu_recipe(
    ...
    contract = {
        "stage": stage,
        "dataset": TARGET_DIRECT_DATASET_DISPLAY[dataset],
        "base_model": "GCN",
        "gu_method": "GNNDelete",
        "seed": seed,
        "ratio": float(ratio),
...
def _small_selection_gu_stage_recipe(stage: str) -> Dict[str, Any]:
    ...
    seed = int(seed_text)
    return {
        "id": f"opengu-small-selection-gu-{stage}-v2",
        "argv": (
        ...
        "recipe_introduced_git_sha": SMALL_SELECTION_GU_V2_RECIPE_INTRODUCED_SHA,
        "git_binding_policy": "job-exact-main-v1",
        "requires_job_expected_git_sha": True,
        "timeout_seconds": RUNNER_AGENT_MAX_TIMEOUT_SECONDS,
...
def _small_selection_gu_v4_stage_artifacts(stage: str) -> Tuple[str, ...]:
    dataset, seed_text = stage.rsplit("-seed", 1)
    seed = int(seed_text)
    paths = []
    for strategy in SMALL_SELECTION_GU_FULL_STRATEGIES:
        leaf = (
            f"{SMALL_SELECTION_GU_V4_FULL_OUTPUT_ROOT}/"
            f"{dataset}_GCN_r0.05/GNNDelete_{strategy}/seed{seed}"
        )
        paths.extend(f"{leaf}/{name}" for name in SMALL_SELECTION_GU_ARTIFACT_NAMES)
    return tuple(paths)
...
def _small_selection_gu_v4_stage_roots(stage: str) -> Tuple[str, ...]:
    return tuple(
        path.rsplit("/", 1)[0]
        for path in _small_selection_gu_v4_stage_artifacts(stage)[::4]
    )


def _small_selection_gu_v4_stage_recipe(stage: str) -> Dict[str, Any]:
    dataset, seed_text = stage.rsplit("-seed", 1)
    seed = int(seed_text)
    return {
        "id": f"opengu-small-selection-gu-{stage}-v4",
        "argv": (
            "{python}", "-m", "experiments.gu_target_v1.syncmate_stage",
            "--config", SMALL_SELECTION_GU_V4_FULL_CONFIG,
            "--stage", stage, "--json",
        ),
        "config_path": SMALL_SELECTION_GU_V4_FULL_CONFIG,
        "config_sha256": SMALL_SELECTION_GU_V4_FULL_CONFIG_SHA256,
        "recipe_introduced_git_sha": SMALL_SELECTION_GU_V4_RECIPE_INTRODUCED_SHA,
        "git_binding_policy": "job-exact-main-v1",
        "requires_job_expected_git_sha": True,
        "timeout_seconds": RUNNER_AGENT_MAX_TIMEOUT_SECONDS,
        "expected_artifact_paths": _small_selection_gu_v4_stage_artifacts(stage),
        "success_predicate": (
            "json.passed == true and generated_artifacts exactly equal "
            "expected_artifact_paths"
        ),
        ...
        "collector_acceptance": True,
        "collector_profile": "small-selection-gu-stage-v1",
        "collector_result_roots": _small_selection_gu_v4_stage_roots(stage),
        "collector_artifact_names": SMALL_SELECTION_GU_ARTIFACT_NAMES,
        "gu_stage": {
            "stage": stage,
            "dataset": dataset,
...
def _small_selection_gu_v5_stage_artifacts(stage: str) -> Tuple[str, ...]:
    dataset, seed_text = stage.rsplit("-seed", 1)
    seed = int(seed_text)
    paths = []
    for strategy in SMALL_SELECTION_GU_FULL_STRATEGIES:
        leaf = (
            f"{SMALL_SELECTION_GU_V5_FULL_OUTPUT_ROOT}/"
            f"{dataset}_GCN_r0.05/GNNDelete_{strategy}/seed{seed}"
        )
        paths.extend(f"{leaf}/{name}" for name in SMALL_SELECTION_GU_ARTIFACT_NAMES)
    return tuple(paths)
...
def _small_selection_gu_v5_stage_roots(stage: str) -> Tuple[str, ...]:
    return tuple(
        path.rsplit("/", 1)[0]
        for path in _small_selection_gu_v5_stage_artifacts(stage)[::4]
    )


def _small_selection_gu_v5_stage_recipe(stage: str) -> Dict[str, Any]:
    dataset, seed_text = stage.rsplit("-seed", 1)
    seed = int(seed_text)
    return {
        "id": f"opengu-small-selection-gu-{stage}-v5",
        "argv": (
            "{python}", "-m", "experiments.gu_target_v1.syncmate_stage",
            "--config", SMALL_SELECTION_GU_V5_FULL_CONFIG,
            "--stage", stage, "--json",
        ),
        "config_path": SMALL_SELECTION_GU_V5_FULL_CONFIG,
        "config_sha256": SMALL_SELECTION_GU_V5_FULL_CONFIG_SHA256,
        "recipe_introduced_git_sha": SMALL_SELECTION_GU_V5_RECIPE_INTRODUCED_SHA,
        "git_binding_policy": "job-exact-main-v1",
        "requires_job_expected_git_sha": True,
        "timeout_seconds": RUNNER_AGENT_MAX_TIMEOUT_SECONDS,
        "expected_artifact_paths": _small_selection_gu_v5_stage_artifacts(stage),
        "success_predicate": (
            "json.passed == true and generated_artifacts exactly equal "
            "expected_artifact_paths"
        ),
        ...
        "collector_acceptance": True,
        "collector_profile": "small-selection-gu-stage-v1",
        "collector_result_roots": _small_selection_gu_v5_stage_roots(stage),
        "collector_artifact_names": SMALL_SELECTION_GU_ARTIFACT_NAMES,
        "gu_stage": {
            "stage": stage,
            "dataset": dataset,
...
RUNNER_RECIPE_DEFINITIONS = {
    ...
    "opengu-small-selection-gu-gate-v2": {
        "id": "opengu-small-selection-gu-gate-v2",
        "argv": (
            "{python}", "-m", "experiments.gu_target_v1.syncmate_recipe",
            "--config", "experiments/configs/syncmate_small_selection_gu_gate_v2.yaml",
            "--json",
        ),
        "config_path": "experiments/configs/syncmate_small_selection_gu_gate_v2.yaml",
        "config_sha256": "342ae520de154dfabf861736448c8c8bcddcccf05a58f247d38706e44aad8dda",
        "recipe_introduced_git_sha": SMALL_SELECTION_GU_V2_RECIPE_INTRODUCED_SHA,
        "git_binding_policy": "job-exact-main-v1",
        "requires_job_expected_git_sha": True,
        "timeout_seconds": RUNNER_AGENT_MAX_TIMEOUT_SECONDS,
        "expected_artifact_paths": SMALL_SELECTION_GU_V2_EXPECTED_ARTIFACTS,
        "success_predicate": (
            "json.passed == true and generated_artifacts exactly equal "
            "expected_artifact_paths"
        ),
        ...
        "collector_acceptance": True,
        "collector_profile": "small-selection-gu-v1",
        "collector_result_roots": (SMALL_SELECTION_GU_V2_OUTPUT_ROOT,),
        "collector_artifact_names": SMALL_SELECTION_GU_ARTIFACT_NAMES,
        "gu_gate": {
            "dataset": "Cora",
            "base_model": "GCN",
    ...
    "opengu-small-selection-gu-gate-v4": {
        "id": "opengu-small-selection-gu-gate-v4",
        "argv": (
            "{python}", "-m", "experiments.gu_target_v1.syncmate_recipe",
            "--config", "experiments/configs/syncmate_small_selection_gu_gate_v4.yaml",
            "--json",
        ),
        "config_path": "experiments/configs/syncmate_small_selection_gu_gate_v4.yaml",
        "config_sha256": "ec0ec36da6d5e65ed72dcd37e8cc2fbd854ffda2da134154e6c566444876264e",
        "recipe_introduced_git_sha": SMALL_SELECTION_GU_V4_RECIPE_INTRODUCED_SHA,
        "git_binding_policy": "job-exact-main-v1",
        "requires_job_expected_git_sha": True,
        "timeout_seconds": RUNNER_AGENT_MAX_TIMEOUT_SECONDS,
        "expected_artifact_paths": SMALL_SELECTION_GU_V4_EXPECTED_ARTIFACTS,
        "success_predicate": (
            "json.passed == true and generated_artifacts exactly equal "
            "expected_artifact_paths"
        ),
        ...
        "collector_acceptance": True,
        "collector_profile": "small-selection-gu-v1",
        "collector_result_roots": (SMALL_SELECTION_GU_V4_OUTPUT_ROOT,),
        "collector_artifact_names": SMALL_SELECTION_GU_ARTIFACT_NAMES,
        "gu_gate": {
            "dataset": "Cora",
            "base_model": "GCN",
    ...
    "opengu-small-selection-gu-gate-v5": {
        "id": "opengu-small-selection-gu-gate-v5",
        "argv": (
            "{python}", "-m", "experiments.gu_target_v1.syncmate_recipe",
            "--config", "experiments/configs/syncmate_small_selection_gu_gate_v5.yaml",
            "--json",
        ),
        "config_path": "experiments/configs/syncmate_small_selection_gu_gate_v5.yaml",
        "config_sha256": "26c1b120c91cc96c14a659e15881687605be9b1e8fedd12aa54e085120e1bd10",
        "recipe_introduced_git_sha": SMALL_SELECTION_GU_V5_RECIPE_INTRODUCED_SHA,
        "git_binding_policy": "job-exact-main-v1",
        "requires_job_expected_git_sha": True,
        "timeout_seconds": RUNNER_AGENT_MAX_TIMEOUT_SECONDS,
        "expected_artifact_paths": SMALL_SELECTION_GU_V5_EXPECTED_ARTIFACTS,
        "success_predicate": (
            "json.passed == true and generated_artifacts exactly equal "
            "expected_artifact_paths"
        ),
        ...
        "collector_acceptance": True,
        "collector_profile": "small-selection-gu-v1",
        "collector_result_roots": (SMALL_SELECTION_GU_V5_OUTPUT_ROOT,),
        "collector_artifact_names": SMALL_SELECTION_GU_ARTIFACT_NAMES,
        "gu_gate": {
            "dataset": "Cora",
            "base_model": "GCN",
...
RUNNER_RECIPE_DEFINITIONS.update(
    {
        f"opengu-small-selection-gu-{stage}-v4": _small_selection_gu_v4_stage_recipe(stage)
        for stage in SMALL_SELECTION_GU_FULL_STAGES
    }
)
RUNNER_RECIPE_DEFINITIONS.update(
    {
        f"opengu-small-selection-gu-{stage}-v5": _small_selection_gu_v5_stage_recipe(stage)
        for stage in SMALL_SELECTION_GU_FULL_STAGES
    }
)
RUNNER_RECIPE_DEFINITIONS.update(
...
def runner_recipe_binding(recipe: str) -> Dict[str, Any]:
    ...
    if definition.get("preflight_profile") == "target-direct-selection-4090-v1" and not errors:
        try:
            ...
            stage = str(
                (definition.get("selection_matrix") or {}).get("stage") or ""
            )
            runtime_preflight = preflight_selection(stage, config_path)
            errors.extend(runtime_preflight.get("errors") or [])
        except Exception as exc:
            errors.append(
                "target-direct Selection runtime preflight failed: "
                "{0}: {1}".format(type(exc).__name__, exc)
            )
    if definition.get("preflight_profile") == "target-direct-gu-4090-v1" and not errors:
        try:
            ...
            runtime_preflight = preflight_gu(
                str(contract.get("stage") or ""), config_path
            )
            errors.extend(runtime_preflight.get("errors") or [])
        except Exception as exc:
            errors.append(
                "target-direct GU runtime preflight failed: "
                "{0}: {1}".format(type(exc).__name__, exc)
            )
    return {
        "recipe": definition,
...
def small_selection_acceptance_payload(
    ...
    *,
    ...
    return {
        "generated_at": now_iso(),
        "mode": (
            "target-direct-selection-acceptance"
            if target_direct
            else "small-selection-acceptance"
        ),
        "node_id": node_id,
...
def small_selection_gu_acceptance_payload(
    ...
    *,
    ...
    target_checkpoint = artifact.get("target_checkpoint") or {}
    ...
    if target_direct:
        ...
        if (
            ...
            errors.append(
                "GU gate target-direct checkpoint/scope provenance is incomplete"
            )
    elif (
        (artifact.get("source_selection") or {}).get("profile")
    ...
    return {
        "generated_at": now_iso(),
        "mode": (
            "target-direct-gu-acceptance"
            if target_direct
            else "small-selection-gu-acceptance"
        ),
        "node_id": node_id,
        "recipe": definition.get("id"),
        "expected_git_sha": expected_git_sha,
...
def small_selection_gu_stage_acceptance_payload(
    ...
    *,
    ...
    for selector in selectors:
        ...
        if target_direct:
            claims = ((meta.get("config") or {}).get("claims") or {})
            state_hash = target_checkpoint.get("state_hash")
            file_sha256 = target_checkpoint.get("file_sha256")
            if (
                ...
                errors.append(
                    "GU stage target-direct checkpoint/scope provenance mismatch: "
                    + str(selector)
                )
            else:
                target_checkpoint_hashes.add((state_hash, file_sha256))
        elif (
            (artifact.get("source_selection") or {}).get("profile")
            != "grandfathered-public-selection-gt-v1"
        ):
    ...
    if target_direct and len(target_checkpoint_hashes) != 1:
        errors.append(
            "target-direct GU stage did not share one exact checkpoint across 17 selectors"
        )
    return {
        "generated_at": now_iso(),
        "mode": (
            "target-direct-gu-stage-acceptance"
            if target_direct
            else "small-selection-gu-stage-acceptance"
        ),
        "node_id": node_id,
        "recipe": definition.get("id"),
        "expected_git_sha": expected_git_sha,
