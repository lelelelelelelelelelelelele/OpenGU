...
5. **Collateral Damage**: **已实现**（代码就绪，待运行实验）— `train_only` flag、`run_retrain()`、`eval_collateral.py`

### Phase A+: Collateral Damage 实验

运行 `eval_collateral.py` 生成 retrain gap + collateral damage 矩阵（6 策略 × 3 方法）。
