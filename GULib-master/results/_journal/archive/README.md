...
The `active4090` snapshot is intentionally untracked user data and exists on
the active server, not in ordinary local clones. Its metadata is carried by
`../auto_report_baseline.json`; a server-side rebuild verifies it when present.
