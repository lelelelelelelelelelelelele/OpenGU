...
\item {\bf Claims}
    ...
    \item[] Justification: The abstract enumerates four contributions (vulnerability spectrum, Shard Protection Effect, collateral diagnostics, mechanism-incomparable robustness) and the introduction (\S\ref{sec:intro}) maps each to a paper section.

\item {\bf Limitations}
    \item[] Question: Does the paper discuss the limitations of the work performed by the authors?
    \item[] Answer: \answerYes{}
    \item[] Justification: \S\ref{sec:conclusion} (Limitations paragraph) discusses (i) selector class incompleteness for MEGU/IDEA, (ii) restricted threat-model scope (node-only, $r\le 0.05$), and (iii) dataset/backbone scope.
...
\item {\bf Experimental result reproducibility}
    ...
    \item[] Answer: \answerYes{}
    \item[] Justification: \S\ref{sec:experiment} specifies datasets, backbones, hyperparameters, seeds, and budget; Appendix~\ref{app:impl} details the IM and TracIn implementations. Code release is committed (\S\ref{sec:experiment}).
...
\item {\bf Experiment statistical significance}
    ...
    \item[] Answer: \answerYes{}
    \item[] Justification: All paired-effect results (\S\ref{sec:results}) report mean, paired standard deviation, 95\% bootstrap CI, and one-sided $t$-test $p$-value over 5 seeds (3 on ogbn-arxiv).
...
\item {\bf Broader impacts}
    ...
    \item[] Answer: \answerYes{}
    \item[] Justification: Discussed in \S\ref{sec:conclusion} (Broader implications): positive impact via robustness norms in GU evaluation; negative consideration of attack-replication risk addressed through framing the work as a diagnostic kit and not releasing pretrained adversarial selectors.
...
\item {\bf Licenses for existing assets}
    ...
    \item[] Answer: \answerYes{}
    \item[] Justification: All five baseline GU methods, the three datasets, and the GNN backbones are cited at first use; we use them as released by their authors under their respective licenses.
