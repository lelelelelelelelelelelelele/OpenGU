...
Next step: 人类复核 `AAGU-006` 的 YAML-driven split-profile 候选与 paired Report，并明确选择接受或返工；接受前不启动正式 GPU 实验。

---

## 0. 一句话现状

当前唯一执行线 `AAGU-006 · FIX · 目标实验 Dataset/Split 权威修复` 已升级到协议 2.1，并形成 YAML-driven split-profile 验证候选与 paired Report，正在等待人类接受或返工；IF 科学定义、实验定义、设备 gate 与后续实验继续沿依赖图等待。
...
- **Current**：`AAGU-006` 已升级到协议 2.1 并形成新的验证候选；当前动作是人的接受/返工决定，接受前不进入 IF Todo Promote、实验定义 gate 或正式 GPU 执行。
