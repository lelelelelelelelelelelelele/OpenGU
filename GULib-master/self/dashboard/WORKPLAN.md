...
Next step: 审阅 AAGU-015 的纯 Selector 配置、分数排名产物和未来 GU 复用小测试；按同一 WorkItem 的 formal 路线决定接受或返工，不调度正式研究矩阵。

---

## 0. 一句话现状

AAGU-015 当前交付实验方案、真实配置及能力覆盖检查；已有 YAML 和定义验证复用，入口支持与缺口由其 WorkItem 材料说明。
...
|---|---|---|---|---|---|
| [AAGU-015](../../.workblock/items/AAGU-015/WORKITEM.md) | EXP | awaiting acceptance / current | P0 | AAGU-006, AAGU-001, AAGU-026, AAGU-009 | [AAGU-015 Block contract](../../.workblock/items/AAGU-015/WORKITEM.md) |
...
- **Current**：当前主推进线为 `AAGU-015` 的当前阶段实验定义材料；生命周期以生成投影为准。
...
- **IF boundary**：`AAGU-015` 交付以纯 Selector 为主的实验方案、对应 YAML、分数排名输出合同与未来 GU 复用的隔离 CPU 小测试，保持 formal 人类验收。独立 Retrain / Metrics 修复与全量实验不归入本次完成条件。
...
flowchart LR
  ...
  subgraph Gates[定义与设备 gate]
    T0[AAGU-015 EXP design and config]
