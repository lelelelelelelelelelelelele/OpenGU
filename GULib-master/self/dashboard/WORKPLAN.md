...
Next step: 先完成 AAGU-034 公共实验 YAML 与统一执行入口修正，作为 026 的后续返工；执行状态以同一 WorkItem 和 live Claim 为准。034 接受后，007 使用统一入口进行最小正式实验，032 消费公共能力完成最终配置验收；031/033 沿既有关系在 007 后平行推进。002 的已接受准备证据及各实验其他前置继续有效。
...
|---|---|---|---|---|---|
| [AAGU-034](../../.workblock/items/AAGU-034/WORKITEM.md) | FIX | awaiting acceptance / current | P0 | AAGU-001, AAGU-026, AAGU-015, AAGU-028 | [AAGU-034 修正合同](../../.workblock/items/AAGU-034/WORKITEM.md) |
| [AAGU-007](../../.workblock/items/AAGU-007/WORKITEM.md) | EXP | blocked by AAGU-034 | P0 | AAGU-002, AAGU-015, AAGU-028, AAGU-034 | [007 ordinary experiment](../../experiments/configs/aagu007/experiment.yaml) |
...
- **Current**：当前优先完成 `AAGU-034` 公共配置与入口 FIX；执行状态以同一 WorkItem 和 live Claim 为准。`AAGU-007` 最小正式实验增加 034 接受前置，`AAGU-032` 最终配置验收也依赖 034；032 的方案讨论可以继续。007 接受后由 `AAGU-031` 展开 015 既定 Selector 矩阵与 Q1–Q4 分析；`AAGU-033` 另消费 `AAGU-032` 已验收方案，与 031 平行完成 D-full 有效性运行。`AAGU-030` 的总表整理不阻塞这两条分支；实时执行状态以同一 WorkItem 与 live Claim 为准。
...
| AAGU-007 | EXP | 最小正式实验 Gate（Target-Direct） | P0 | AAGU-002, AAGU-015, AAGU-028, AAGU-034 | [007 ordinary experiment](../../experiments/configs/aagu007/experiment.yaml) |
