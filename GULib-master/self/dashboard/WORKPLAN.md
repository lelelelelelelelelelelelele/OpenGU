...
Next step: 审阅 AAGU-015 当前阶段实验方案、实际 YAML 与能力覆盖说明；验证后由用户决定接受或返工，不调度全量实验或自动补齐全部实现。
...
- **IF boundary**：`AAGU-015` 交付当前阶段的两阶段实验方案、对应 YAML 及能力覆盖说明，保持 formal 人类验收。代码缺口的后续实现与全量实验、正式成本、科学结论不自动归入本次完成条件。
