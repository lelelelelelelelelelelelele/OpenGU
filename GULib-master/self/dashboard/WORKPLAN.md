...
Next step: AAGU-015 已从 Todo 原地 Promote 为两阶段 EXP Block，保持 registered / not claimed。先完成并接受 AAGU-026 与 AAGU-009 的前置候选；之后在独立任务中 Claim 同一 AAGU-015 locator，先交付 YAML 与无写入矩阵展开，再进入正式实验门槛。

---

## 0. 一句话现状

AAGU-006 与 AAGU-001 已接受。AAGU-015 已登记为 Selector-only 与固定 Selection→Unlearning 两阶段 EXP Block，覆盖三小图、17 个 Selector、时间证据和配对 Retrain；当前未 Claim 或运行，并等待 AAGU-026、AAGU-009 前置被接受和落地。
...
| [AAGU-015](../../.workblock/items/AAGU-015/WORKITEM.md) | EXP | blocked by AAGU-026 | P0 | AAGU-006, AAGU-001, AAGU-026, AAGU-009 | [AAGU-015 Block contract](../../.workblock/items/AAGU-015/WORKITEM.md) |
...
- **Current**：当前主推进线仍为 `AAGU-026` 的配置与缓存实现；`AAGU-015` 已完成 Block formation，保持 registered / not claimed，待 026 与 009 前置满足后从同一 locator Claim。生命周期以生成投影为准。
