...
Next step: 在同一 `AAGU-006` 中把 split default 改为可覆盖的合法配置，并证明等价合同只物化一次、不同合同保留多样性；不启动正式 GPU 实验。

---

## 0. 一句话现状

当前唯一执行线 `AAGU-006 · FIX · 目标实验 Dataset/Split 权威修复` 正在按验收意见返工：70/10/20 保留为默认值，但不再冻结为唯一允许比例；同合同归一复用、不同合同保留多样性。IF 科学定义、实验定义、设备 gate 与后续实验继续沿依赖图等待。
...
|---|---|---|---|---|---|
| [AAGU-006](../../.workblock/items/AAGU-006/WORKITEM.md) | FIX | awaiting acceptance / current | P0 | — | [target-direct formal v2 recipe](../../experiments/configs/syncmate_target_direct_formal_v2.yaml) |
...
- **Current**：`AAGU-006` 正在同一 locator 内返工 split identity；当前动作是通用合法 split 的归一化、注册审计与真实复用测试，完成前不进入 IF Todo Promote、实验定义 gate 或正式 GPU 执行。
