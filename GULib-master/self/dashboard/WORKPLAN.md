...
Next step: AAGU-006 已接受并关闭；AAGU-015 成为下一项可形成的 IF 科学定义 Todo，需在独立任务中先升级同一 WorkItem 到当前协议，再 Promote/Claim，不能在本次 Closeout 顺带执行。

---

## 0. 一句话现状

AAGU-006 已接受：70/10/20 是当前默认 split，同合同归一复用、不同合法合同保留多样性，删除预算由实验注册独立拥有。下一编排节点是 AAGU-015 的 IF 科学定义；实验定义、设备 gate 与后续实验继续沿依赖图等待。
...
| [AAGU-003](../../.workblock/items/AAGU-003/WORKITEM.md) | GATE | blocked by AAGU-014, AAGU-010, AAGU-023 | P2 | AAGU-014, AAGU-010, AAGU-023 | [AAGU-003 Block contract](../../.workblock/items/AAGU-003/WORKITEM.md) |
...
- **Current**：`AAGU-006` 已接受关闭；`AAGU-015` 是下一项可形成的 IF 科学定义 Todo，但本次 Closeout 不替它升级、Promote、Claim 或作科学选择。
