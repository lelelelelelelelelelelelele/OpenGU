...
Next step: 人类复核 `AAGU-006` 的已验证候选并明确选择接受或返工；只有接受后才由同一 Block locator 进入 `block-closeout`，不重复 claim、实施或启动正式 GPU 实验。

---

## 0. 一句话现状

当前唯一执行线 `AAGU-006 · FIX · 目标实验 Dataset/Split 权威修复` 已形成验证候选，正在等待人类接受或返工；IF 科学定义、实验定义、设备 gate 与后续实验继续沿依赖图等待。
...
- **Current**：`AAGU-006` 已完成实现与本地 Verify；当前动作是人的接受/返工决定，接受前不进入 IF Todo Promote 或实验定义 gate。
