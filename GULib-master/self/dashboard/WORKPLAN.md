...
Next step: AAGU-034 已接受并合入；在用户明确授权前，AAGU-007 保持 registered / not claimed。获授权后，007 使用统一入口完成最小正式实验；032 消费公共能力完成最终配置验收；031/033 沿既有关系在 007 后平行推进。002 的已接受准备证据及各实验其他前置继续有效。

---

## 0. 一句话现状

AAGU-034 公共配置与统一执行入口 FIX 已接受并合入，修正 026 的未收敛实现并接管 015 冗余配置生成；AAGU-007 现为当前最小正式实验 gate，但仍未 Claim 或获运行授权，032 的最终配置验收也已可消费该修正。AAGU-015 的 Selector 方案与能力检查已接受；实际实验先走 AAGU-002 设备就绪 → AAGU-007 最小正式实验，再由 AAGU-031 展开 Selector 大组与分析；AAGU-033 在 032 方案验收后独立承接 D-full 有效性运行，与 031 平行。Phase 2 由 AAGU-030 整理完整表，AAGU-011/AAGU-012 已沿用原编号改登记为 X3 GU 主矩阵与结果、X4 退化分解与副作用分析。其余实验分区与配置继续逐项映射，各 Block 生命周期由下方 WorkItem 投影给出。
...
|---|---|---|---|---|---|
| [AAGU-007](../../.workblock/items/AAGU-007/WORKITEM.md) | EXP | registered / not claimed / current | P0 | AAGU-002, AAGU-015, AAGU-028, AAGU-034 | [007 ordinary experiment](../../experiments/configs/aagu007/experiment.yaml) |
...
已关闭 FIX 节点已从活动投影隐藏：9 个；历史保留在对应 WorkItem 与 Git。
<!-- WORKITEM_STATUS:END -->
...
- **Current**：`AAGU-034` 已接受并合入，当前线转为 `AAGU-007` 最小正式实验。007 仍为 `registered / not claimed`，必须先获得明确运行授权；`AAGU-032` 可消费 034 的已接受公共能力完成最终配置验收。007 接受后由 `AAGU-031` 展开 015 既定 Selector 矩阵与 Q1–Q4 分析；`AAGU-033` 另消费 `AAGU-032` 已验收方案，与 031 平行完成 D-full 有效性运行。`AAGU-030` 的总表整理不阻塞这两条分支；实时执行状态以同一 WorkItem 与 live Claim 为准。
