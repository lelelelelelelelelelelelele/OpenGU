...
| **环境** | 🟢 本地分析就绪 / SSH RTX 4090 正常 | H→E 已重指；本地 conda `gnn` 继续用于 CPU 分析，本地 RTX 5070 与 pinned torch 不兼容。旧 153-cell GU matrix 已完成并收口，但只保留为 L1 surrogate-transfer 证据；新 target-direct formal GPU cell 尚未启动 | `reports/target_direct_selection_PREPARATION_REPORT.md` |
| **数据安全** | 🟢 已多处备份 | 服务器有完整原件；本地备份 `D:\backups\OpenGU_GULib\2026-06-15\` + `F:\…`（2482 文件校验通过） | 备份 MANIFEST |
| **代码** | 🟡 target-direct 修正待验收 | 旧 run 固定 `main@1c83bb4`，artifact/provenance 有效但科学身份降级。严格白盒修正在 `codex/fix-target-direct-selection-20260723`：ScoreBundle 与 GU 绑定同一 checkpoint/state hash，真实 expected-k 与 candidate fail closed；尚未合入 main | git；`reports/target_direct_selection_PREPARATION_REPORT.md` |
...
| **E8** ★ | **小图 17-Selection → GNNDelete target-direct 白盒重做**：同一 OpenGU GCN checkpoint 同时用于 ScoreBundle 与 GU；70/10/20 无泄漏 split；真实 5% train-candidate budget；回答 TracIn vs degree/random 与 IF formula effect/feasibility | `experiments/target_direct_v1/*`; 待合入 main 后生成 pinned SyncMate config | G1 data preflight + G2 17-way selection + G3 1-cell GU gate + 153-cell matrix；612 artifacts | C1 / selector×GU interaction | ◐ 修正实现与 `183 passed` 定向测试完成；旧 153-cell 矩阵降级为 L1 transfer screen；待分支验收、no-ff merge、SSH gate/full |
...
- **2026-07-22/23** E8 旧矩阵完成后重分类：`153/153` cells、`612/612` artifacts、0 failures 与逐文件 SHA-256 仍成立；但由于 GateGCN hidden16/200-epoch selector 与 OpenGU GCN hidden64/100-epoch target 不是同一 state，且目录标 5% 实际 exact `k=7`，所有效果数字只保留为 L1 surrogate-transfer / engineering evidence。严格 target-direct 修正已在隔离分支实现：同一 checkpoint 双哈希绑定、70/10/20 validation target、真实 5% expected-k、candidate/count fail-closed、cold/warm/VRAM/failure 计时合同；尚未合入 main 或启动 formal GPU gate。见 `reports/target_direct_selection_PREPARATION_REPORT.{md,html}`。
