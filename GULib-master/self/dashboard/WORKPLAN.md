...
Next step: 先执行 AAGU-036 结果存储修复，接受并落地后再授权 AAGU-007 最小正式实验；031/033 及后续实际实验沿 007 继承此前置。方案整理、无写入检查和修复所需隔离软件验证可先行，本次编排不授权实验运行。
...
已关闭 FIX 节点已从活动投影隐藏：10 个；历史保留在对应 WorkItem 与 Git。
<!-- WORKITEM_STATUS:END -->
...
flowchart LR
  subgraph Repair[修复队列]
    ...
    R9[AAGU-036 FIX shared Dataset/Split output storage]
    R3 --> R4
  end
...
| AAGU-036 | FIX | 实验结果引用既有 Dataset/Split，消除重复存储 | P0 | AAGU-034 | [AAGU-036 修复合同](../../.workblock/items/AAGU-036/WORKITEM.md) |
