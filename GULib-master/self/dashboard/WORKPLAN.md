...
Next step: 在同一 `AAGU-006` linked source 中提交 YAML-driven split-profile 实现，在 clean candidate 上运行 registered dry-run，并重建 paired acceptance report；不启动正式 GPU 实验。

---

## 0. 一句话现状

当前唯一执行线 `AAGU-006 · FIX · 目标实验 Dataset/Split 权威修复` 正在原地返工：WorkItem 已升级到协议 2.1，YAML-driven split-profile 实现已通过本地回归，正在形成新的 clean candidate；IF 科学定义、实验定义、设备 gate 与后续实验继续沿依赖图等待。
...
|---|---|---|---|---|---|
| [AAGU-006](../../.workblock/items/AAGU-006/WORKITEM.md) | FIX | in progress / current | P0 | — | [target-direct formal v2 recipe](../../experiments/configs/syncmate_target_direct_formal_v2.yaml) |
...
| [AAGU-019](../../.workblock/items/AAGU-019/WORKITEM.md) | FIX | registered / not claimed | P0 | AAGU-018 | [AAGU-019 Block contract](../../.workblock/items/AAGU-019/WORKITEM.md) |
...
- **Current**：`AAGU-006` 已升级到协议 2.1 并完成实现级本地 Verify；当前动作是 clean candidate dry-run 与新验收报告，接受前不进入 IF Todo Promote 或实验定义 gate。
