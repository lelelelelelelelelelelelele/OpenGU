...
Current node: AAGU-005

Next step: AAGU-005 已整理 OpenGU 接入交付与现有证据；下一步对齐正式 GU 的预检、产物声明及接受检查，具体缺口由同一 WorkItem 与报告拥有。SM-005 的 Core 已接受，015 与 028 的软件和材料接受范围由各自 WorkItem 拥有。

---

## 0. 一句话现状

AAGU-001、AAGU-026、AAGU-009 与 AAGU-028 已接受；015 已接受本阶段方案与能力检查。当前整理 AAGU-005 的消费端交付，并处理其接入核对发现的剩余项；全量研究实验运行不属于此次整理。各 Block 生命周期由下方 WorkItem 投影给出，未合入的候选以其同一 WorkItem 和 live Claim 为准。
...
|---|---|---|---|---|---|
| [AAGU-005](../../.workblock/items/AAGU-005/WORKITEM.md) | SUPPORT | in progress / current | P3 | AAGU-001 | [AAGU-005 Block contract](../../.workblock/items/AAGU-005/WORKITEM.md) |
