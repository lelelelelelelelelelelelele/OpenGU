...
已投 NeurIPS、在等审稿、**rebuttal 还远 = 完善期**。回流数据 `degree ≥ IM/IF`（raw-F1 和 retrain gap 都是）——但**这没证伪贡献**：投出去的 thesis 本就是 *systematic audit + extreme heterogeneity + fingerprint*，不是"IM/IF 最强"。2026-07-22 的 153-cell GNNDelete 矩阵已于 2026-07-23 重分类为 **public-split、exact-k=7、L1 surrogate-transfer / engineering screen**：selector 与实际 GU target 不是同一 checkpoint，不能继续作为 target-direct 白盒结论。`target_direct_v1` 基础修正已通过 `--no-ff` 合入 `main`；正在准备的 E8 编排保留 70/10/20 无泄漏 validation profile，并把删除预算修正为 train candidates 的 1%/5%。恢复 SSH GPU、`gnn_20` 和三个 processed profiles 后，必须分别通过 Cora/seed42/degree 的 1% 与 5% gate；306-cell 候选扩展当前未获执行授权。准备合同见 `reports/target_direct_selection_PREPARATION_REPORT.{md,html}`。
...
1. **C1 — 指标与实验身份均需修正，非贡献被证伪**：`degree ≥ IM/IF` 的既有 raw-F1 / retrain-gap 证据仍是 paper liability；但新增 E8 数字 `degree +2.30 pp`、TracInCP point `+0.17 pp` 只能作为 public-split、exact-k=7、L1 surrogate-transfer screen，不能再强化 target-direct 白盒 claim。严格比较必须让 selector 与 GU 复用同一 target checkpoint，在 70/10/20 validation-target split 上分别按真实 1%/5% train candidates 删除；split 比例不是删除预算。thesis 本是 *audit + 异质性*，不是"IM/IF 最强"；新矩阵仍需按 dataset 与 ratio 分层，不允许 Cora-only pooled 结论。→ `reports/target_direct_selection_PREPARATION_REPORT.md`。
...
Cache V2 Selection Artifact 真实命中 ─► E7 两组独立 gate ─┬─ Group 1 `proper-tracin-v1`：GCN → GAT/GIN
                                                        └─ Group 2 `d-gif-sgc-v1`：SGC → GCN/GAT/GIN
...
| **E7** ★ | **两组 query-free surrogate-transfer（独立门控）**：**Group 1** 用 GCN selector 迁移到 GAT / GIN target GNNDelete；**Group 2** 用 SGC analytical selector 迁移到 GCN / GAT / GIN target GNNDelete。每个 victim 的 target-direct 选集只作 white-box reference，不算第三组；不做 GCN-B→GCN-A。报告 top-k Jaccard / overlap / 可用时的 rank correlation，以及 victim 上原始 utility、loss、GU outcome 和相对 target-direct 的配对差值；transfer ratio 仅作描述，不预设必须放大 approximation gap | 待建 `E7_gcn_cross_backbone.yaml`、`E7_sgc_analytical_surrogate.yaml`；Group 1 gate = `proper-tracin-v1` + Cache V2 cold/warm exact hit；Group 2 gate = `d-gif-sgc-v1` + SGC selector 单元验证 + Cache V2 cold/warm exact hit；显式记录 Selection Artifact ref，且 `selector_model_id != target_model_id` | Group 1 = 2 victims × 5 seeds = 10 个最小身份对；Group 2 = 3 victims × 5 seeds = 15 个最小身份对；最终 YAML 另行显式展开 selector / ratio / baseline。两组分别通过最小 registered gate 后暂停审核，互不以 60% 阈值解锁 | L2-direct → L2-surrogate / threat-model realism | ◐ generic 17-output Cache V2 real-hit 已由 SSH `9/9` grandfathered GT 特例关闭，**不再要求 3×3 重跑**。Group 1 仍待 `proper-tracin-v1` 与 cross-backbone runner；Group 2 仍待 `d-gif-sgc-v1`、SGC selector 与 runner。正式 GPU cell 未获授权；设计见 [E7 代理选集迁移实验计划](../../../../OpenGU-DocMap/10_实验矩阵/24_E7代理选集迁移实验计划.md) |
| **E8** ★ | **小图 17-Selection → GNNDelete target-direct 白盒重做**：同一 OpenGU GCN checkpoint 同时用于 budget-independent ScoreBundle 与 GU；70/10/20 是无泄漏 split，1%/5% 是独立删除预算；公共 `utils/node_split.py` 统一原生 OpenGU 与 formal staging 的 deterministic split；回答 TracIn vs degree/random 与 IF formula effect/feasibility | `experiments/target_direct_v1/*`; `experiments/configs/syncmate_target_direct_formal_v2.yaml` | G1 data preflight + G2 每 dataset/seed 一次 cold ScoreBundle、两套 ratio-conditioned Selection Artifacts + G3 两个 Cora/seed42/degree GU gates；候选完整矩阵 306 cells / 1,224 artifacts，当前未授权 | C1 / selector×GU interaction | ◐ 基础 target-direct 修正由 merge commit `71076bc` 验收；双预算与公共 split 已接受到当前主线；旧 153-cell 矩阵降级为 L1 transfer screen；2026-07-24 target/Cache V2/SyncMate 复核 `316 passed`，SSH 无 GPU/`gnn_20`，且 70/10/20 processed profiles 缺失，formal jobs=0 |

**坑提醒**：E1 的 2 月 citeseer 数据在 `_archive_20260506/` 是**污染数据，不能引**；改 GNN 架构维度才需清 `data/{Method}/`，E1–E6 不改架构故不用清。public Planetoid 固定 split（Cora/CiteSeer/PubMed train=`140/120/60`、val=`500`）不是 OpenGU canonical 80/20 processed split，二者结果不得互相改名。E8 是 direct selected-set → GNNDelete screen；在 E7 中它只提供各 victim 的 white-box reference，不能关闭 surrogate cell 的 `selector_model_id != target_model_id`。E7 仅含 GCN→GAT/GIN 与 SGC→GCN/GAT/GIN 两组，不做 GCN-B→GCN-A，也不把 SGC 自身的理论保证外推为非凸 victim 的迁移保证。E7 禁止通过“清 IF / selection cache”切换算法口径：Legacy IF / Selection Cache 全程只读；算法换版创建带显式 algorithm / producer version 的新 V2 Recipe，旧 V2 Artifact 不删除、不覆盖，只有明确退役时才显式 retire。
...
| **W6** | **review collection**：汇总 评审/导师/AI 意见，建一个 **reviewer意见文件区**（放 OB/Obsidian 最好）。已有 `report/advisor_report_2026-06-16.html` + [AI review](../../../../OpenGU-DocMap/30_评审与汇报/AI审稿_2026-06-28.md)（**5/10 weak reject**；三痛点已映射到 E1·E5 / W1·W3 / C2·A7） | — | — | ☐ |
...
- **全项目文档地图**：[OpenGU DocMap](../../../../OpenGU-DocMap/_文档地图.md)（MOC + 零散 md 归集处）
...
- **2026-06-28** 接入 [AI review](../../../../OpenGU-DocMap/30_评审与汇报/AI审稿_2026-06-28.md)（5/10 weak reject，**非官方审稿** → 现状仍完善期）→ W6；三痛点（scope 没跑完 / 叙述过满 / 统计偏弱）对应 E1·E5 / W1·W3 / C2·A7（均已在计划内）；W3 加"清未完成语句"；§3 fingerprint 偏轻被 review 印证。
...
- **2026-08-05** E7 设计覆盖旧 C.6a/C.6b 顺序门：删除同架构 GCN-B→GCN-A 和 `transfer ratio ≥60%` 解锁关系，改为两组独立 gate——Group 1 GCN→GAT/GIN，Group 2 SGC→GCN/GAT/GIN；target-direct 只作 white-box reference，transfer ratio 只作描述，不预设放大 approximation gap。→ [E7 代理选集迁移实验计划](../../../../OpenGU-DocMap/10_实验矩阵/24_E7代理选集迁移实验计划.md)。
...
- **2026-07-22/24** E8 旧矩阵完成后重分类并修订预算轴：`153/153` cells、`612/612` artifacts、0 failures 与逐文件 SHA-256 仍成立；但由于 GateGCN hidden16/200-epoch selector 与 OpenGU GCN hidden64/100-epoch target 不是同一 state，且目录标 5% 实际 exact `k=7`，所有效果数字只保留为 L1 surrogate-transfer / engineering evidence。严格 target-direct 基础修正已实现同一 checkpoint 双哈希绑定和 70/10/20 validation target；当前准备线进一步把删除预算改为 1%/5%，每个 ratio 从 verified train candidate count 独立推导 expected-k，并用一次 immutable ScoreBundle 派生两套 ratio-conditioned Selection Artifacts。两个 Cora/seed42/degree gate 均未启动；306-cell 候选扩展未授权。见 `reports/target_direct_selection_PREPARATION_REPORT.{md,html}`。
- **2026-07-24** E8 deterministic split 收归 OpenGU 公共层：新增无 `config.py` import 副作用的 `utils/node_split.py`，原生 `transductive_split_node` 与 target-direct formal staging 共用同一 ratio/count、randperm、mask、indices 和 induced-edge 实现；`split_profile.py` 仅保留 canonical materialize/verify、manifest/hash、互斥/穷尽与 Selection identity。70/10/20、split seed 2024、模型 seeds 共用一份 split 和 1%/5% train-candidate budget 均未改变；本地未生成正式 profile/cache/result。
