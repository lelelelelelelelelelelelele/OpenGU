...
| **W6** | **review collection**：汇总 评审/导师/AI 意见，建一个 **reviewer意见文件区**（放 OB/Obsidian 最好）。已有 `report/advisor_report_2026-06-16.html` + OpenGU DocMap 评审部分收录的 AI review（**5/10 weak reject**；三痛点已映射到 E1·E5 / W1·W3 / C2·A7） | — | — | ☐ |
...
- **全项目文档地图**：OpenGU DocMap（MOC + 零散 md 归集处）
...
- **2026-06-28** 接入 OpenGU DocMap 评审部分收录的 AI review（5/10 weak reject，**非官方审稿** → 现状仍完善期）→ W6；三痛点（scope 没跑完 / 叙述过满 / 统计偏弱）对应 E1·E5 / W1·W3 / C2·A7（均已在计划内）；W3 加"清未完成语句"；§3 fingerprint 偏轻被 review 印证。
- **2026-06-28** 建 OpenGU DocMap 文件区（零散 md 的家 + 全项目分类 MOC）；根目录两份零散 md（规划手记、AI 审稿）归入并加 frontmatter；`daily-log` / `progress` 不动、只索引。
...
- **2026-08-05** E7 设计覆盖旧 C.6a/C.6b 顺序门：删除同架构 GCN-B→GCN-A 和 `transfer ratio ≥60%` 解锁关系，改为两组独立 gate——Group 1 GCN→GAT/GIN，Group 2 SGC→GCN/GAT/GIN；target-direct 只作 white-box reference，transfer ratio 只作描述，不预设放大 approximation gap。详见 OpenGU DocMap 的实验部分。
