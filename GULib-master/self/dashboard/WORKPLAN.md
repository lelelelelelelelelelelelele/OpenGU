...
- **2026-08-05** E7 设计覆盖旧 C.6a/C.6b 顺序门：删除同架构 GCN-B→GCN-A 和 `transfer ratio ≥60%` 解锁关系，改为两组独立 gate——Group 1 GCN→GAT/GIN，Group 2 SGC→GCN/GAT/GIN；target-direct 只作 white-box reference，transfer ratio 只作描述，不预设放大 approximation gap。→ `文档规划/10_实验矩阵/24_E7代理选集迁移实验计划.md`。
