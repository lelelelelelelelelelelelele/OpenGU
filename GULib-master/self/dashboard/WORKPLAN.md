...
Current node: AAGU-036

Next step: AAGU-038 已并入 AAGU-036；先讨论并确认回传产物合同，再完成 Result/Cache 分离与共享输入引用的统一验证和验收。既有提交与测试保留为部分进度；后续受影响的新实验须待036接受落地，已有运行、分析和证据保留。
...
|---|---|---|---|---|---|
| [AAGU-036](../../.workblock/items/AAGU-036/WORKITEM.md) | FIX | awaiting acceptance / current | P0 | AAGU-034 | [AAGU-036 修复合同](../../.workblock/items/AAGU-036/WORKITEM.md) |
...
已关闭 FIX 节点已从活动投影隐藏：11 个；历史保留在对应 WorkItem 与 Git。
<!-- WORKITEM_STATUS:END -->
...
- **Current**：先完成 `AAGU-036` 结果存储修复并接受落地，再进入 `AAGU-007` 最小正式实验；031/033 与其他实际实验继承 007 的前置。032/030 方案工作与修复隔离验证可先行。登记和依赖调整不等于 Claim 或运行授权。
