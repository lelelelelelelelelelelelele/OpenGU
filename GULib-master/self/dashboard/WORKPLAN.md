...
Current node: AAGU-026

Next step: 用户已接受 AAGU-001 公共合同。完成其收口后，分别开启 AAGU-026 实现任务与 AAGU-015 selector-only 实验工作流/协议表讨论任务：026 验证真实缓存行为，015 给出具体科研样例供确认；两者可并行推进，不因本次接受而启动正式实验。

---

## 0. 一句话现状

AAGU-006 与 AAGU-001 已接受。AAGU-026 消费三层参数合同，实现配置与缓存身份隔离；AAGU-015 并行形成 selector-only 的真实工作流、实验协议表和预期 HIT/MISS 样例，供研究负责人讨论确认。公共规范被接受不等于实现已经完成或本轮实验已获批准。
...
|---|---|---|---|---|---|
| [AAGU-026](../../.workblock/items/AAGU-026/WORKITEM.md) | FIX | registered / not claimed / current | 未定 | AAGU-001 | [AAGU-026 Block contract](../../.workblock/items/AAGU-026/WORKITEM.md) |
| [AAGU-009](../../.workblock/items/AAGU-009/WORKITEM.md) | FIX | awaiting acceptance | P0 | — | [AAGU-009 软件修复合同](../../.workblock/items/AAGU-009/WORKITEM.md) |
...
- **Current**：当前主推进线为 `AAGU-026` 的配置与缓存实现；`AAGU-015` 在独立任务中并行准备真实 selector-only 工作流和协议表。旧 001 任务只负责收口和移交，不实施下一 Block。生命周期以生成投影为准。
...
- **Implementation boundary**：用户已授权另开任务执行 `AAGU-026`，落实独立配置与方法级缓存身份；Claim 和实现由新任务负责，不将实现完成作为 001 的前置。
- **IF boundary**：`AAGU-015` 仍保留科学定义 Todo；新任务先给出本轮 selector-only 的实际工作流、具体参数表、指标/证据与预期缓存变化样例，再讨论完整性和 Promote。科学选择、正式执行与真实结果分析不由本次任务移交代替批准。
