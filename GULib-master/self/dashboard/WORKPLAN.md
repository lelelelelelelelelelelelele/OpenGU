...
| **E8** ★ | **小图 17-Selection → GNNDelete target-direct 白盒重做**：同一 OpenGU GCN checkpoint 同时用于 budget-independent ScoreBundle 与 GU；70/10/20 是无泄漏 split，1%/5% 是独立删除预算；回答 TracIn vs degree/random 与 IF formula effect/feasibility | `experiments/target_direct_v1/*`; `experiments/configs/syncmate_target_direct_formal_v2.yaml` | G1 data preflight + G2 每 dataset/seed 一次 cold ScoreBundle、两套 ratio-conditioned Selection Artifacts + G3 两个 Cora/seed42/degree GU gates；候选完整矩阵 306 cells / 1,224 artifacts，当前未授权 | C1 / selector×GU interaction | ◐ 基础 target-direct 修正由 merge commit `71076bc` 验收；双预算编排在准备分支验证中；旧 153-cell 矩阵降级为 L1 transfer screen；SSH 无 GPU/`gnn_20`，且 70/10/20 processed profiles 缺失，formal jobs=0 |
