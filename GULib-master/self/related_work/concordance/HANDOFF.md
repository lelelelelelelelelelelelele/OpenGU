...
real GIF (s=H⁻¹∇L_E via LiSSA), proper TracIn ⟨∇ℓ_v,∇L_E⟩, deployed cross-TracIn
⟨∇ℓ_v,−Σ∇ℓ⟩, self ‖∇ℓ‖. Here T is the deletion candidate pool; E is only the target
evaluation/query set whose loss change is predicted. Test split is used as E only for this diagnostic
comparison, not as a set of nodes to select for unlearning.

| pair | cora (F1=.88) | citeseer (F1=.73) | pubmed (F1=.86) |
|---|---|---|---|
| GIF ↔ **TracIn-proper** ⟨∇ℓ,∇L_E⟩ (FIX) | **0.742** | **0.727** | **0.647** |
...
1. **The cheap fix recovers IF fidelity.** Proper TracIn (contract with ∇L_E, *Hessian-free*, same cost as
...
2. **⚠ Mechanism correction.** An earlier claim that "Σ∇ℓ≈0 at convergence → noise" was WRONG: measured ‖Σ∇ℓ‖
   ...
   direction** (parameter/regularisation, not held-out loss sensitivity) → ranks by the wrong criterion.
...
∇L_val/query, not the training-gradient residual). Affects only TracIn + Hybrid; re-run just those cells if you want corrected attack
