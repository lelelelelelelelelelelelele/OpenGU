...
A one-line change — contract each node gradient with the **test/query** loss gradient instead of
the **training-gradient sum** — recovers **0.65–0.74** overlap with GIF at *identical* cost
...
## 1. Three influence estimators

For a trained model with parameters θ̂, per-node loss ℓ_v, training set T, and a held-out
evaluation set E:

| name | score for candidate v | Hessian? | cost |
|---|---|---|---|
| **deployed cross-TracIn** | −⟨∇ℓ_v, Σ_{j∈T} ∇ℓ_j⟩ | no | O(\|T\|·d) |
| **proper TracIn** (the fix) | ⟨∇ℓ_v, ∇L_E⟩ where L_E = Σ_{e∈E} ℓ_e | no | O(\|T\|·d) |
| **GIF** (real graph IF) | ⟨∇ℓ_v, H⁻¹∇L_E⟩ | yes (LiSSA) | O(\|T\|·d) + HVP iters |
...
`Σ_j ∇ℓ_j = ∇L_T` — the **training-loss gradient sum**. The proper TracIn (Pruthi et al. 2020,
single-checkpoint, influence on the *evaluation* loss) contracts with `∇L_E`. GIF (Wu et al. 2023)
is the same as proper TracIn but with the inverse-Hessian preconditioner.

All three cost the same per-candidate backward pass; GIF adds one cheap LiSSA solve for
`s = H⁻¹∇L_E` (then a dot product per candidate). The deployed and proper forms differ **only in
the reference vector they contract with**.
...
vector pointing along the **parameter / regularization-residual direction**, which has nothing to
do with descent on the evaluation loss. The deployed cross-TracIn therefore ranks nodes by how much
their gradient aligns with the *aggregate-training / regularization* direction — **the wrong
criterion** — not by influence on test performance. It is not noise; it is a deterministic ranking
of the wrong thing. The proper TracIn contracts with `∇L_E`, the actual direction of evaluation-loss
change, which is why it tracks GIF.
...
```python
# deployed (mis-specified): contract with the training-gradient sum (~ -|T| λ θ)
col_sum = G.sum(dim=0)              # Σ_j ∇ℓ_j  ≈ regularization residual
scores  = -(G @ col_sum)
