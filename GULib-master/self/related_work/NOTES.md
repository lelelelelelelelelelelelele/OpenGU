...
- **GIF-as-scorer（历史）**：早期 34%-acc GNNDelete checkpoint feasibility 结果已被 trained base-GCN diagnostic 取代；旧称 “proper TracIn” 的 single-final proxy 标签已经退役。见 [[related-work-zhang-chen-2026]]、`concordance/FINDING_tracin_misspecification.md`；当前验证边界详见 OpenGU DocMap 的实验部分。
