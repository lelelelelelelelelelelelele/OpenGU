...
- **GIF-as-scorer（历史）**：早期 34%-acc GNNDelete checkpoint feasibility 结果已被 trained base-GCN diagnostic 取代；旧称 “proper TracIn” 的 single-final proxy 标签已经退役。见 [[related-work-zhang-chen-2026]]、`concordance/FINDING_tracin_misspecification.md` 和[当前实验入口](../../../../OpenGU-DocMap/10_实验矩阵/12_近似策略重合度实验.md)。
