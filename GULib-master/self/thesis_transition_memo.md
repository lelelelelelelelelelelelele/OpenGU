...
> Current source: 当前研究方向见 [OpenGU DocMap](../../../OpenGU-DocMap/_文档地图.md)，任务编排见 [WorkBlock / Companion 记录](../.workblock/items/)；正文的下一步和执行授权只属于记录时的上下文。
...
| `mia_auc`（JSON 字段；paper 称 update-detection AUC） | **不为 0.000，且 ∈ [0.3, 0.9]** | update-detection 路径未生效（legacy "MIA bug"）—— 回查 patch |
| `mia_auc != NaN` | True | update-detection evaluator 异常（legacy: "MIA evaluator"），看 log |
