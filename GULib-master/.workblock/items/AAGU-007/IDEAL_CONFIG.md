...
可直接查看 [experiment.ideal.yaml](experiment.ideal.yaml)。正式组合表已位于 `experiments/configs/aagu007/experiment.yaml`，科学范围与本稿一致。034 已接受并合入统一入口，但正式表仍使用相对路径；本稿的小表引用只写文件名、由字段类型固定定位公共目录的规则尚未实现。剩余差异与执行基线见 [WorkItem](WORKITEM.md#034-收口后的执行基线--2026-09-07)。
...
本稿可以检查 YAML 语法、引用形式以及逻辑矩阵为四个方法实例；不能宣称文件名引用已通过当前解析器的 dry-run。034 已完成并接受；固定公共目录识别补齐后，须由统一入口核对公共引用、最终有效值与来源、配置指纹、执行注册及 Result 合同。当前相对路径正式表和本理想稿的验证状态分别记录。真实提交仍遵循本轮配置审阅和运行批准。
