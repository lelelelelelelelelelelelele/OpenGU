...
def main():
    ...
    increment = (f"已形成三个 Dataset/Split、17 个 Selector、GNNDelete/GIF 与评价实例，生成 {counts['stage_s']} 个阶段 S cell "
                 f"和 {counts['stage_u']} 个阶段 U 候选 cell。424 份生成 YAML 由已接受的模块化 parser 解析；本报告只呈现配置检查点，正式实验尚未开始。")
    observations = [
        ('PASS（配置结构）', '三个 Dataset/Split、17 个 Selector、两种 GU 和评价引用均可解析；未知字段、错误类型引用及生成物漂移会被拒绝。真实输入尚未绑定。'),
        ('PASS（无写入展开）', '324 张真实计划表展开为 306 个 Selector cell 和 612 个 GU cell；两个 experiments/run.py CLI dry-run 成功。展开未读数据、调用 producer 或创建研究结果。'),
        ('NOT OBSERVED', '正式 data/split/candidate、checkpoint、Recipe/Artifact、job 和设备身份尚未绑定。测试确认未绑定 Dataset/Split 会在模型、Store 和结果写入前停止。'),
        ('NOT OBSERVED', '没有本轮 Score、排名或 Selection 原始产物，因此没有计算 Spearman、Kendall、top-K common fraction 或 Jaccard，也没有形成 Q1–Q4 的科学结论。'),
        ('NOT OBSERVED', '没有运行 cold compute、warm access、Selection materialization、共享准备或资源测量。配置展开与测试耗时不作为 Selector 成本。'),
        ('NOT CONFIRMED', 'U 计划只声明固定 selection_input，没有 selector_refs；但当前模块化 GU 缺完整重训练消费者，retrain-gap case 实际被拒绝。真实固定 Selection→GU→Retrain/Evaluation 链尚未验证。'),
        ('NOT OBSERVED（完整科研报告）', '本 Markdown/HTML 对应真实配置验证证据；技术准备、实测观察和科学判断分别标注。完整研究结果报告仍待实验及收集核验。'),
        ('NOT CONFIRMED（用户决定）', '用户尚未决定配置先行落地顺序，也未接受整个 EXP。Claim 保持 ongoing；矩阵展开和测试通过不触发研究验收。'),
    ]
    decision = ('Agent 判断：证据不足，不能接受整个 EXP。当前需要用户明确同一 015 的配置/入口先行落地顺序：'
                '项目要求正式实验使用三端同一已落地 main，而本 Block 的最终 formal 接受需要实验结果。'
                '决定该顺序后，仍需在同一 015 补齐正式 launcher、完整重训练与评价、真实输入和 canary/成本门槛；本检查点不申请 612-cell 调度。')
    defaults = [
    ...
    verification = ('配置检查点 59baa2ae909e7fba92278d9201c635b80be65cdc 上，8 项针对性回归、324 个 parser 计划、'
                    ...
                    '后续仅报告/渲染器与 Record 说明变化时复用该检查点的产品验证，并单独校验当前报告与最终 HEAD 差异。')
    render_note = ('HTML 视觉检查：PASS（已观察范围）。Chrome headless 实际渲染并查看 1440×1100 桌面首屏、'
                   '600×1800 窄屏和 1440×3300 完整报告；标题、正文、表格和决定区可读，无可见重叠或横向截断。'
                   '430px 的首个 CLI 截图出现右侧截断，原因未单独验证，不作为 PASS 证据；未声称更窄视口通过。'
                   '截图只支持配置检查点的报告可读性，不构成研究证据。')
    ...
    md += [caveat, '']
    md_table('正式门槛核验', ('门槛', '状态', '实际事实'), gates)
    ...
    body += '<section><h2>正式门槛核验</h2>' + table(('门槛', '状态', '实际事实'), gates) + '</section>'
