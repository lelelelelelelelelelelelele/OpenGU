...
def main():
    ...
    observations = '五 Selector × 两 GU × 一 Evaluation 的计划可完整 dry-run；B-Hutch 32 小表只写 last_layer，64 小表只多 probes: 64。真实 CPU 消费者中，改变 Evaluation 只改变 Evaluation receipt，Selection 与 GU Result 均 HIT；普通 modular lane 引用缺少 exact-retrain 输入的 retrain-gap case 会在写 Store 前失败。设备与 Torch/CUDA build 只进入执行回执，不再进入 Score/Checkpoint 身份。'
    decision = '建议接受本 Block 的返工候选。122 项 CPU／集成检查、182 项 SyncMate 检查通过，主项目 3,990 个历史结果文件及所列缓存目录前后哈希一致。由用户决定接受、继续返工或拒绝；正式 GPU、SSH 部署和科学结果接纳仍未执行。'
    scenarios = [
        ('真实组合表与简写小表', '可解析计划明确列出 5 个 selector_refs、2 个 unlearning_refs 和 1 个 evaluation_ref。B-Hutch 32 沿用注册默认 probes=32，只显式写 last_layer；B-Hutch 64 只增加 probes=64。完整有效参数及来源仍进入 dry-run／运行证据。', 'PASS', 'experiment_five_selectors_two_gu.yaml / test_minimal_method_files_expand_real_defaults'),
