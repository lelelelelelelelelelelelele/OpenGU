...
TESTS = [
    'tests/test_modular_consumers.py', 'tests/test_generic_cache_v2.py',
...
def main():
    ...
    parser.add_argument('--syncmate-python', required=True)
    ...
    commands = [
        ...
        ('syncmate', [args.syncmate_python, '-B', '-X', 'utf8', '-m', 'pytest', '--noconftest',
                      'tests/test_syncmate.py', '-q', '--tb=short', '--junitxml=' + str(output/'syncmate.xml')]),
    ]
    results, consumers = [], []
    ...
    receipt = {'candidate':candidate, 'source': str(repo), 'checks': results, 'protected_roots':protected,
               ...
               'raw_evidence_directory': str(output)}
