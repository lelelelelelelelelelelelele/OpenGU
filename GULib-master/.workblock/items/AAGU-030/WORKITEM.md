...
本项归下一研究阶段。当前阶段优先运行并分析 015 已确定的 Selector 实验；它不等待本项完成。011/012 已按用户要求在原编号下改登记为 X3 GU 主矩阵与结果、X4 退化分解与副作用分析，本项将其具体组合与原表对齐。其余既有 007/008/013/014/027 等任务按可复用、需调整或尚缺承接核对，不重复注册一套编号。
...
- 011/012 已改为 X3/X4，不再是独立 replacement。CiteSeer 原 A5 数据集范围和 GraphRevoker 方法维度仍须保留在全量重跑表中；当前登记确定责任，本项继续完成具体坐标和配置映射。
...
- 2026-09-06：用户要求 011/012 沿用原编号改登记为 X3/X4；同步本项映射基线，保留原实验表覆盖责任，未执行本项。
