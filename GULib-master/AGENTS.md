...
Current research framing and scientific rationale start in the sibling [OpenGU DocMap](../../OpenGU-DocMap/_文档地图.md). [Work Plan](self/research/index.html) independently owns experiment creation, preparation, runs, reruns, analysis, scientific decisions and experiment dependencies; its continuously maintained local state is `self/research/experiments/AAGU-NNN.json` (Git-ignored), while versioned analysis and scientific decisions live in `self/research/analyses/AAGU-NNN.json`. State updates never require a commit; completed analysis is committed separately with the original execution SHA, then the next formal run re-establishes three-end SHA alignment. Development contracts and lifecycle belong to canonical WorkBlock records and Companion. An experiment may reference a blocking development Block; Work Plan never manages or synchronizes Block lifecycle.
