...
本轮只使用 `parameter_scope=last_layer`。它仍是 target-direct white-box，因为参数来自同一实际 target checkpoint，但明确属于“last-layer IF approximation”，不能写成 full-parameter exact IF。

`all_trainable` 不配置、不调度，也不是本轮扩展条件。依据是现有 IF 文献已经明确记录全参数 Hessian/IHVP 的扩展成本，以及实际工作中选择层级或最后一层的可行性路径；本轮只需如实限定 claim，而不再支付独立 stress ladder 的计算成本。文献依据与边界集中列在 `target_direct_formal_preflight_AUDIT_REPORT.md` 的 “Why `last_layer` is retained” 一节。未来若研究全参数 IF，必须另立成本/近似合同与全新 Cache V2 identity。
...
需要的下一步授权是：验收本分支后提交，并以显式 `--no-ff` 合入 `main`。合并且基础设施恢复后，先做 G1→G2，再分别执行 1% 与 5% 的 G3 formal gate。两个 gate 都通过也只完成 canary 验收；G4/G5 的 306-cell 候选扩展仍需用户另行明确授权，不会自动启动。远端正式 results 必须逐文件 SHA-256 回传到本地作为分析输入。
