...
class GATv2Net(abstract_model):
    def __init__(self,args,in_channels, out_channels, num_layers=2, dropout=0.5):
        ...
        self.convs.append(GATv2Conv(in_channels, 8, heads=8, dropout=self.dropout))
        if self.args["downstream_task"]=="graph":
            self.convs.append(GATv2Conv(8 * 8, hidden_channels))
            self.linear = torch.nn.Linear(hidden_channels,out_channels)
        else:
            self.convs.append(GATv2Conv(8 * 8, out_channels, heads=1, concat=False, dropout=self.dropout))
