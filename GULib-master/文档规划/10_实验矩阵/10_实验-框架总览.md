...
| 近似策略重合度：IM / IF 这类近似策略是否仍有效 | approximation axis | [[12_近似策略重合度实验]] |
...
| GraphRevoker repair | GU method axis | [[13_重跑与缓存修复Runbook]] |
...
2. **TracIn refresh**：校正 IF 轴，重新解释 TracIn / Hybrid。
3. **GraphRevoker repair**：恢复 6-method audit 完整性。
...
先读 [[11_策略输出重合度实验]] 和 [[12_近似策略重合度实验]]。IF 线继续按 [[20_IF目标层级对比实验计划]] → [[21_C目标TracIn与GIF近似有效性实验计划]] 的顺序阅读：先分清 A / B / C，再验证 C 内部的 TracIn / GIF 近似。
