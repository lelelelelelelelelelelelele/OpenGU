...
- `results/_journal/auto_report.md`: append-only historical v1/v2 research journal (never migrate/rewrite)
- `results/_journal/auto_report.events.jsonl`: append-only V3 machine event audit stream
- `results/_journal/auto_report_status.md` + `.html`: bounded, rebuildable current-state projection (not audit originals)
...
- Historical v1/v2 Markdown stays append-only; legacy write/read APIs remain compatible.
...
- Decision/session Markdown entries remain manual v2 records when needed.
