...
现行普通YAML规范见[configs/README.md](configs/README.md)，统一命令为 `experiments/run.py`。本地验证使用独立临时CPU目录；正式作业经SyncMate登记和项目预检进入同一解析/执行内核。当前任务与批准范围见 canonical WorkItem / Companion；配置与结果入口见 实验研究总览。
