"""Run an isolated Cora gate for the UNSTABLE TracIn V2 scorer.
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--lr-milestones", default="10,20")
...
def model_parameters(model: GateGCN, scope: str) -> List[nn.Parameter]:
    if scope == "last_layer":
        return [parameter for parameter in model.conv2.parameters() if parameter.requires_grad]
    return [parameter for parameter in model.parameters() if parameter.requires_grad]


def parameter_schema_hash(model: GateGCN, scope: str) -> str:
    entries = []
...
def main(argv=None) -> None:
    ...
    model = GateGCN(dataset.num_features, args.hidden_channels, dataset.num_classes, args.dropout).to(device)
    started = time.time()
    ...
    k = max(int(candidate_ids.numel() * args.topk_ratio), 1)
    selections = {
    ...
    result = {
        "gate_schema_version": "tracin-v2-gate-report-v1",
        "stability": "unstable",
