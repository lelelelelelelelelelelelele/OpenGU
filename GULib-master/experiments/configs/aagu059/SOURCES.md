...
本表检验作者求解参数在当前 OpenGU 两层 GCN、hidden=64/16 上的迁移；保留当前训练、原图测试集和 Random 10% 节点删除（抽样 seed104245，训练 seed42）。不是完整作者实验复刻。
...
执行入口是注册 recipes `opengu-aagu059-table01-h64-v3` 与 `opengu-aagu059-table01-h16-v4`，分别消费 table01.yaml 和 table01_hidden16.yaml，每份六格，总计十二格。输出独立数值诊断 JSON，通过 SyncMate Core 传输和 SHA-256 索引验证，随后执行项目诊断检查。它不发布 GU Cache Output；运行完成不代表收敛、科研接受或用户验收。
