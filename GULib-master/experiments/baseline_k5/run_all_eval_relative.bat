...
REM ------------------------------------------------------------------
REM  MG-0: Cora / GCN / GIF, GNNDelete, GraphEraser
REM ------------------------------------------------------------------
if not defined MISSING_ONLY (
    echo.
    echo [MG-0] Cora / GCN  (3 methods x 5 seeds)
)
for %%M in (GIF GNNDelete GraphEraser) do (
    for %%S in (%SEEDS%) do (
        if not defined MISSING_ONLY echo   -- %%M / cora / GCN / seed=%%S %REPAIR_ARG%
...
for %%M in (GIF GNNDelete GraphEraser) do (
    for %%S in (%SEEDS%) do (
        if not defined MISSING_ONLY echo   -- %%M / citeseer / GCN / seed=%%S %REPAIR_ARG%
...
for %%M in (GIF GNNDelete GraphEraser) do (
    for %%S in (%SEEDS%) do (
        if not defined MISSING_ONLY echo   -- %%M / cora / GAT / seed=%%S %REPAIR_ARG%
...
for %%M in (GIF GNNDelete) do (
    for %%R in (0.01 0.05 0.10 0.20) do (
        for %%S in (%SEEDS%) do (
            if not defined MISSING_ONLY echo   -- %%M / cora / GCN / ratio=%%R / seed=%%S %REPAIR_ARG%
            %PYTHON% %SCRIPT% --dataset_name cora --base_model GCN --unlearning_methods %%M --strategies %STRATEGIES% --unlearn_ratio %%R --random_seed %%S --baseline_k 5 %REPAIR_ARG%
        )
    )
)
